...
export class Helper {
  ...
  static delaySimple = (ms, acc = 'BOT', msg = 'Sleep...', type = 'START') => {
    return new Promise((resolve) => {
      let remainingMilliseconds = ms;

      this.templateLog(acc, `${msg}`, type);
  ...
  static templateLog(account = 'BOT', msg = 'Sleep', type = 'INFO') {
    const message = `| ${colors.cyan(type)} | ${colors.yellow('[Blum]')} | ${account} | ${msg}`
    console.log(message)
  }
